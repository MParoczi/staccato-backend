...
- **FR-016**: The application MUST scan and register all AutoMapper profiles from the `Application` and `Api` project assemblies.
...
- **FR-019**: Each configuration group MUST be bound to a dedicated strongly typed options class (`JwtOptions`, `AzureBlobOptions`, `CorsOptions`, `RateLimitOptions`) via the options pattern, with `.ValidateDataAnnotations()` and `.ValidateOnStart()` so that any misconfiguration causes an immediate startup failure with a descriptive error rather than a silent runtime defect.
...
- **CorsOptions**: Encapsulates `AllowedOrigins` (`string[]`). An empty array is valid (all cross-origin requests rejected); a null value prevents the application from starting. Values must be specific origin strings — wildcards are incompatible with `AllowCredentials()`.
