...
1. **Given** an authenticated user, **When** they call GET /users/me, **Then** the response returns their full profile including displayName, email, preferredLanguage, defaultPageSize, defaultInstrumentId, avatarBlobPath, and scheduledDeletionAt (null if not scheduled).
2. **Given** an authenticated user, **When** they call PUT /users/me with valid values for displayName, preferredLanguage, defaultPageSize, and defaultInstrumentId, **Then** the response returns 200 with the updated profile.
3. **Given** an authenticated user, **When** they call PUT /users/me with an invalid defaultPageSize value or with any of the four required fields missing, **Then** the response returns 400 with field-level validation errors.
...
**Independent Test**: Can be fully tested by uploading a valid image via PUT /users/me/avatar, confirming the response returns 200 and the user's profile now includes an updated avatarBlobPath reference.
...
6. **Given** an authenticated user with an existing avatar, **When** they call `DELETE /users/me/avatar`, **Then** the response returns 204, the blob is removed from storage, and `GET /users/me` returns null for `avatarBlobPath`.
...
6. **Given** an authenticated user, **When** they reference a preset that belongs to another user, **Then** the response returns 403.
7. **Given** an unauthenticated request, **When** any preset endpoint is called, **Then** the response returns 401.
...
- **FR-001**: System MUST allow authenticated users to retrieve their full profile (displayName, email, preferredLanguage, defaultPageSize, defaultInstrumentId, avatarBlobPath, scheduledDeletionAt) via GET /users/me.
- **FR-002**: System MUST allow authenticated users to update their displayName, preferredLanguage, defaultPageSize, and defaultInstrumentId via PUT /users/me. All four fields are required on every call (full replacement semantics); omitting any field MUST return 400. Sending `null` for `defaultPageSize` or `defaultInstrumentId` clears the preference (sets the value to null in storage).
...
- **FR-011**: System MUST reject avatar uploads in formats other than JPG, PNG, and WebP with a 400 validation error.
...
- **FR-013a**: System MUST allow authenticated users to remove their avatar entirely via `DELETE /users/me/avatar`; the response MUST be 204, the blob MUST be deleted from storage, and `avatarBlobPath` MUST be set to null. Calling this endpoint when no avatar is set MUST return 204 (idempotent).
...
- **FR-016**: System MUST reject preset creation with fewer than 12 style definitions, more than 12, or duplicate module types, with a 400 validation error. `moduleType` values are matched case-insensitively against the `ModuleType` enum; unrecognised values MUST return 400.
...
- **FR-022**: System MUST run a background cleanup process on a daily schedule that permanently deletes all user accounts whose ScheduledDeletionAt is in the past, along with all cascade-associated data and avatar blobs.
- **FR-023**: Background cleanup MUST log failures on individual account deletions and continue processing remaining accounts; a single failure MUST NOT halt the entire cleanup run.
...
- **SC-003**: Avatar uploads for valid files under 2MB complete successfully 100% of the time under normal conditions; invalid uploads are rejected immediately with actionable error messages.
...
- The `preferredLanguage` field accepts only the two supported locales: `en` and `hu`.
- Style preset names must be non-empty strings; a maximum length of 100 characters is assumed as a reasonable default.
...
- Q: Does `PUT /users/me` require all four fields or allow partial updates? → A: All four fields required on every call (full replacement / standard PUT semantics); omitting any returns 400.
